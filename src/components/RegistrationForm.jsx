...
const RegistrationForm = ({ handleRegister }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      // navigate('/login'); // Assuming 'navigate' is defined elsewhere
    } catch (err) {
      ...
      <div className='mt-8 md:flex md:justify-center md:mt-16'>
        <form onSubmit={handleSubmit(onSubmit)} className='grid gap-y-6 w-full md:w-[468px]'>
          <div className='flex flex-col gap-y-2'>
            ...
            <input {...register('username')} type='text' id='username' autoComplete='off' />
            {errors.username && (
          ...
          <div className='flex flex-col gap-y-2'>
            ...
            <input {...register('email')} type='text' id='email' autoComplete='off' />
            {errors.email && (
